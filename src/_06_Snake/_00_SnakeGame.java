...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	public void startGame() {
		...
		String instructions = "Use the arrow keys to change direction";
	...
	private void gameOver() {
		...
		Location startLocation = new Location (WIDTH/2,HEIGHT/2);
		
		//1. stop the timer
		timer.stop();
		//2. tell the user their snake is dead
		JOptionPane.showMessageDialog(null, "The snake is dead");
		//3. ask them if they want to play again.
		if(JOptionPane.showInputDialog("Would you like to play again (Y for yes, N for no").equals("Y")) {
			snake.reset(startLocation);
			timer.start();
		}else {
			window.dispose();
		}
